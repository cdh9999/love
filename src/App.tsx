...
function App() {
  ...
  const [dhiaKaoutharFirstTime, setDhiaKaoutharFirstTime] = useState(true);
  ...
  const calculateLove = () => {
    ...
    setTimeout(() => {
      ...
      if (isJalil && isMaria) {
        // Special case: 99.99% for Jalil & Maria
        percentage = 99.99;
        ...
        status = 'high';
      } else if (isDhia && isKaouthar) {
        // Special case for Dhia & Kaouthar: 95% first time, then random
        if (dhiaKaoutharFirstTime) {
          percentage = 95;
          setDhiaKaoutharFirstTime(false);
          const firstTimeComments = [
            'واو! 🌟 نسبة عالية بزاف! راكم على الطريق الصحيح! 95% هاذي نسبة ممتازة! 💕',
            'ممتاز! 🎯 95%! الكمبيوتر يقول راكم مناسبين بزاف لبعضاكم! 🚀',
            'يا سلام! 🎉 95% هاذي نسبة خرافية! راكم قريبين من الكمال! 💎',
            'مبروك! 🌈 95%! النتيجة تقول راكم مثاليين تقريباً! كملوا هكا! ✨',
            'رائع! 🔥 95%! هاذي نسبة تخلي القلب يفرح! راكم محظوظين! 💖'
          ];
          comment = firstTimeComments[Math.floor(Math.random() * firstTimeComments.length)];
          status = 'high';
        } else {
          // Random percentage for subsequent times
          ...
          // Apply normal logic for random results
          if (percentage === 0) {
